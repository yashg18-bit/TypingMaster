...
async function saveResultToBackend(wpm, accuracy) {
...
const coderProfileName = document.getElementById("coderProfileName");

if (username) {
    const initial = username.charAt(0).toUpperCase();

    if (profileName) profileName.innerText = initial;
    if (coderProfileName) coderProfileName.innerText = initial;
}
...
async function loadBestResult() {
  ...
  try {
    ...
    const bestWpm = document.getElementById("n-best-wpm");
    const bestAcc = document.getElementById("n-best-acc");

    if (bestWpm) bestWpm.textContent = data.wpm;
    if (bestAcc) bestAcc.textContent = data.accuracy + "%";
...
async function loadHistory() {
  ...
  try {
    ...
    const testsTaken =
      document.getElementById("n-tests-taken");

    if (testsTaken) {
      testsTaken.textContent = data.length;
    }
